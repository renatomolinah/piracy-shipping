...
# Identify the id for each lhs variable
dist_ids <- fixest::models(mod) |>
  filter(rhs == "TNE3",
         lhs == "distance_km") |>
  pull(id)

time_ids <- fixest::models(mod) |>
  filter(rhs == "TNE3",
         lhs == "time_hours") |>
  pull(id)

n_vessel_ids <- fixest::models(mod) |>
  filter(rhs == "TNE3",
         lhs == "n_vessels") |>
  pull(id)

n_trips_ids <- fixest::models(mod) |>
  filter(rhs == "TNE3",
         lhs == "n_trips") |>
  pull(id)

dist <- mod[dist_ids] |>
  set_names(c("Full sample", "GoA", "GoG", "SEA"))
time <- mod[time_ids] |>
  set_names(c("Full sample", "GoA", "GoG", "SEA"))
n_vessel <- mod[n_vessel_ids] |>
  set_names(c("Full sample", "GoA", "GoG", "SEA"))
n_trips <- mod[n_trips_ids] |>
  set_names(c("Full sample", "GoA", "GoG", "SEA"))

msummary(list("Panel (A): Total Distance (km)" = dist,
              "Panel (B): Occupancy (hr)" = time,
              "Panel (C): Voyages (\\#)" = n_vessel,
              "Panel (D): Vessels (\\#)" = n_trips),
         coef_omit = "Intercept",
