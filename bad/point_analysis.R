...
#standardSQL
  ...
  SELECT
    ...
    trip_id
  FROM
    `ucsb-gfw.piracy.voyage_ais_positions_bad`)
SELECT
  EXTRACT(date
  FROM
    timestamp) date,
...
bq_table(project = project,table = "point_analysis_gridded_shipping_hours_bad",dataset = "piracy") %>% 
  bq_table_delete()
bq_project_query(project,query = sql, destination_table =  bq_table(project = project,table = "point_analysis_gridded_shipping_hours_bad",dataset = "piracy"),
                 use_legacy_sql = FALSE, allowLargeResults = TRUE)
...
WITH
  ...
  FROM
    `ucsb-gfw.piracy.asam_bad`),
  distinct_grids AS(
  SELECT
    DISTINCT lon_bin,
    lat_bin
  FROM
    `ucsb-gfw.piracy.point_analysis_gridded_shipping_hours_bad`),
  joined_table AS(
  SELECT
    *
  FROM
...
bq_table(project = project,table = "point_distance_lookup_bad",dataset = "piracy") %>% 
  bq_table_delete()
bq_project_query(project,query = sql, destination_table =  bq_table(project = project,table = "point_distance_lookup_bad",dataset = "piracy"),
                 use_legacy_sql = FALSE, allowLargeResults = TRUE)
...
bq_table(project = project,table = "point_attacks_crossed_bad",dataset = "piracy") %>% 
  bq_table_delete()
bq_project_query(project,query = sql, destination_table =  bq_table(project = project,table = "point_attacks_crossed_bad",dataset = "piracy"),
                 use_legacy_sql = FALSE, allowLargeResults = TRUE)
...
sql<- "#standardSQL
  ...
  SELECT
    *
  FROM
    `ucsb-gfw.piracy.point_distance_lookup_bad`
...
bq_table(project = project,table = "point_analysis_full_bad",dataset = "piracy") %>% 
  bq_table_delete()
bq_project_query(project,query = sql, destination_table =  bq_table(project = project,table = "point_analysis_full_bad",dataset = "piracy"),
                 use_legacy_sql = FALSE, allowLargeResults = TRUE)
...
sql<-"#standardSQL
  ...
  FROM
    `ucsb-gfw.piracy.point_analysis_full_bad`
  GROUP BY
    attack_reference,
    date_attack,
...
bq_table(project = project,table = "point_summary_shipping_bad",dataset = "piracy") %>% 
  bq_table_delete()
bq_project_query(project,query = sql, destination_table =  bq_table(project = project,table = "point_summary_shipping_bad",dataset = "piracy"),
                 use_legacy_sql = FALSE, allowLargeResults = TRUE)
...
sql<-"#standardSQL
  ...
  FROM
    `ucsb-gfw.piracy.point_attacks_crossed_bad`
  GROUP BY
    attack_reference,
    date_attack,
    days_since_attack_bin,
...
bq_table(project = project,table = "point_summary_attacks_bad",dataset = "piracy") %>% 
  bq_table_delete()
bq_project_query(project,query = sql, destination_table =  bq_table(project = project,table = "point_summary_attacks_bad",dataset = "piracy"),
                 use_legacy_sql = FALSE, allowLargeResults = TRUE)
...
WITH
  ...
  SELECT
    *
  FROM
    `ucsb-gfw.piracy.point_summary_shipping_bad`),
  attack_info AS(
  SELECT
    *
  FROM
    `ucsb-gfw.piracy.point_summary_attacks_bad`)
SELECT
...
bq_table(project = project,table = "point_analysis_summary_bad",dataset = "piracy") %>% 
  bq_table_delete()
bq_project_query(project,query = sql, destination_table =  bq_table(project = project,table = "point_analysis_summary_bad",dataset = "piracy"),
                 use_legacy_sql = FALSE, allowLargeResults = TRUE)
...
master AS(
SELECT * 
FROM `piracy.point_analysis_summary_bad` 
WHERE 
...
bq_table(project = project,table = "point_analysis_cumulative_bad",dataset = "piracy") %>% 
  bq_table_delete()
bq_project_query(project,query = sql, destination_table =  bq_table(project = project,table = "point_analysis_cumulative_bad",dataset = "piracy"),
                 use_legacy_sql = FALSE, allowLargeResults = TRUE)

point_analysis_cumulative <- bq_project_query(project, "SELECT * FROM `piracy.point_analysis_cumulative_bad`") %>%
  bq_table_download(max_results = Inf)

write_csv(point_analysis_cumulative,path="processed_data/point_analysis_bad.csv")
