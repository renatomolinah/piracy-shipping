...
ais_info AS(
  ...
  {clusters_aggregated}
  FROM
  `piracy.voyage_ais_positions_bad`
  GROUP BY
  mmsi,
  lat_bin,
  lon_bin,
...
COS(RADIANS(direction_degrees - heading)) * speed_m_s wind_vector
  ...
  USING(mmsi,year)
")
bq_table(project = project,table = "voyages_gridded_bad",dataset = "piracy") %>% 
  bq_table_delete()
bq_project_query(project,query = sql, destination_table = bq_table(project = project,table = "voyages_gridded_bad",dataset = "piracy"),
