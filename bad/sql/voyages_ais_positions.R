...
bq_table(project = project,table = "voyage_ais_positions_bad",dataset = "piracy") %>% 
  bq_table_delete()
bq_project_query(project,query = sql, destination_table =  bq_table(project = project,table = "voyage_ais_positions_bad",dataset = "piracy"),
                 use_legacy_sql = FALSE, allowLargeResults = TRUE)
