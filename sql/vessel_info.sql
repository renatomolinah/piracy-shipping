...
# Load time-varying version of vessel database
  ...
  GROUP BY
    mmsi),
  # Finally, determine which mmsi have are registered as cargo acrosss all years
...
IF(REGEXP_CONTAINS(registry_vessel_type,'cargo|cargo_or_tanker|bunker_or_tanker|tanker|cargo_or_reefer|specialized_reefer|container_reefer|reefer|bunker'
), TRUE, FALSE) vessel_type_is_cargo_registry,
IF(mmsi IN (SELECT mmsi FROM vessel_consistently_cargo_across_years),TRUE,FALSE) vessel_type_is_cargo_registry_across_years
FROM
vessel_info_base),
# make the most restrictive sample, where we look for vessels that are consistently registered as cargo in the time-varying vessel info tables based on official registries
vessel_info_base_by_year AS(
  SELECT
CAST(ssvid AS INT64) mmsi,
year,
ARRAY_TO_STRING(registry_info.best_known_vessel_class,";") registry_vessel_type
...
WHERE (vessel_type_is_cargo_best OR vessel_type_is_cargo_registry OR vessel_type_is_cargo_registry_across_years)