...
subs <- function(subset, name) {
  ...
  select(id, reference, description = descriptio) |>
  mutate(keep = "",
         attacker = "",
         attacked = "",
         notes = "")
...
dim(subset)[1] == length(unique(c(Grayson$reference, Sebastian$reference, Felipe$reference, Alejandro$reference, Ariella$reference)))



# Lets combine (duplicate) all the data. Each reference should appear only twice
combined <- bind_rows(Grayson, Sebastian, Felipe, Alejandro, Ariella)
...
# X ----------------------------------------------------------------------------
write_csv(x = subset,
          file = here("data", "intern_data", "master.csv"))
write_csv(x = Grayson,
          file = here("data", "intern_data", "Grayson.csv"))
write_csv(x = Sebastian,
          file = here("data", "intern_data", "Sebastian.csv"))
write_csv(x = Felipe,
          file = here("data", "intern_data", "Felipe.csv"))
write_csv(x = Alejandro,
          file = here("data", "intern_data", "Alejandro.csv"))
write_csv(x = Ariella,
          file = here("data", "intern_data", "Ariella.csv"))
